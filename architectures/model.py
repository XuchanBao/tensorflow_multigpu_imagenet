...
class model:
    ...
    def __init__(self, inputs, labels, loss, optimizer, wd, architecture, num_classes, is_training, transfer_mode,
                 top_n=5, max_to_keep=5, num_gpus=1):
    ...
    def get_grads(self, device):
        ...
        with tf.device(device):
            ...
            last_grads = self.optimizer.compute_gradients(total_loss,
                                                          var_list=tf.get_collection(SAVE_VARIABLES, scope='output'))
        ...
        with tf.variable_scope(tf.get_variable_scope()):
            for i in range(self.num_gpus):
                with tf.name_scope('Tower_%d' % i) as scope:
                    ...
                    # all towers.
                    grads, last_grads, batchnorm_updates, cross_entropy_mean, top1acc, topnacc = self.get_grads(
                        '/gpu:%d' % i)
    ...
    def train_ops(self):
        ...
        if self.transfer_mode[0] == 3:
            train_op = tf.cond(
                tf.less(tf.get_collection(SAVE_VARIABLES, scope="epoch_number")[0], self.transfer_mode[1]),
    ...
    def inference(self):
        if self.architecture.lower() == 'alexnet':
            return alexnet(self.inputs, self.num_classes, self.wd, tf.where(self.is_training, 0.5, 1.0),
                           self.is_training)
            ...
            return googlenet(self.inputs, self.num_classes, self.wd, tf.where(self.is_training, 0.4, 1.0),
                             self.is_training)
