...
def do_train(sess, args):
    ...
    with tf.device('/cpu:0'):
        ...
        dnn_model = model(images_ph, labels_ph, utils.loss, optimizer, wd, args.architecture, args.num_classes,
                          ...
                          num_gpus=args.num_gpus, max_to_keep=args.num_save_model_segments + 2)
        ...
        train_loader = loader(args.train_info, args.delimiter, args.raw_size, args.processed_size, True,
                              args.chunked_batch_size, args.num_prefetch, args.num_threads, args.path_prefix,
                              args.shuffle)
        ...
        if args.run_validation:
            val_loader = loader(args.val_info, args.delimiter, args.raw_size, args.processed_size, False, batch_size_tf,
                                args.num_prefetch, args.num_threads, args.path_prefix)
            ...
            if args.run_validation:
                ...
                for step in range(args.num_val_batches):
                    ...
                    val_img, val_lbl = sess.run([val_images, val_labels], feed_dict={
                        batch_size_tf: args.num_val_samples % min(512,
                                                                  args.batch_size)} if step == args.num_val_batches - 1 else None,
                    ...
                    if step % 10 == 0 or step == args.num_val_batches - 1:
                        print("Validation step %d of %d, Loss %.2f, Top-1 Accuracy %.2f, Top-%d Accuracy %.2f " % (
                        step, args.num_val_batches, total_loss.avg, top1.avg, args.top_n, topn.avg))
                        sys.stdout.flush()
...
def do_evaluate(sess, args):
    with tf.device('/cpu:0'):
        ...
        dnn_model = model(images_ph, labels_ph, utils.loss, None, 0.0, args.architecture, args.num_classes,
                          is_training_ph, args.transfer_mode)
        ...
        if not args.inference_only:
            ...
            for step in range(args.num_val_batches):
                ...
                val_loss, top1_predictions, topn_predictions, topnguesses, topnconf = sess.run(eval_ops, feed_dict={
                    images_ph: val_img, labels_ph: val_lbl, is_training_ph: False},
                                                                                               options=args.run_options,
                                                                                               run_metadata=args.run_metadata)
                ...
                print(
                    ...
                    (step, args.num_val_batches, top1.sum, args.top_n, topn.sum, total_loss.avg, top1.avg, args.top_n,
                     topn.avg))

                # log results into an output file
                for i in range(0, val_inf.shape[0]):
                    out_file.write(predictions_format_str % (
                    step * args.batch_size + i + 1, val_inf[i], val_loader.label_dict[val_lbl[i]],
        ...
        # inference
            ...
            for step in range(args.num_val_batches):
                ...
                topnguesses, topnconf = sess.run(eval_ops, feed_dict={images_ph: val_img, is_training_ph: False},
                                                 options=args.run_options, run_metadata=args.run_metadata)
...
def main():  # pylint: disable=unused-argument
    ...
    parser.add_argument('--batch_size', default=128, type=int, action='store',
                        help='The batch size for training, evaluating, or inference')
    ...
    parser.add_argument('--train_info', default='train_imgs.txt', action='store',
                        help='Name of the file containing addresses and labels of training images')
    parser.add_argument('--val_info', default='val_imgs.txt', action='store',
    ...
    parser.add_argument('--LR_policy', default='piecewise_linear',
                        help='LR change policy type (piecewise_linear, constant, exponential)')
    parser.add_argument('--WD_policy', default='piecewise_linear',
                        help='WD change policy type (piecewise_linear, constant, exponential)')
    parser.add_argument('--LR_details', default='19, 30, 44, 53, 0.01, 0.005, 0.001, 0.0005, 0.0001',
                        help='LR change details')
    ...
    parser.add_argument('--optimizer', default='momentum',
                        help='The optimization algorithm (SGD, Momentum, Adam, RMSprop, ...)')
    parser.add_argument('--top_n', default=5, type=int, action='store', help='Specify the top-N accuracy')
    parser.add_argument('--max_to_keep', default=5, type=int, action='store',
                        help='Maximum number of snapshot files to keep')
